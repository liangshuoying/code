...
#                 print('running state hack', state2.shape)
        ...
        # save begin
        if conf['save_dir'] != "" and ((j+1) % conf['save_interval'] == 0 or \
                    (final_rewards.mean()>50 or final_rewards.max()>50) and\
...
#     args.hack_state = True
    ...
    conf['env_args'] = dict(obs_pixel=0, obs_size=7, act_disc=0, obs_win=4, obs_dtype=None)
